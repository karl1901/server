...
- 4、打开**test中的nginx的配置文件**,**复制前两条指令**  
  - nginx的VsCode插件  
  - ![img49](img/bs/微信截图_20210429142504.png)  
...
- 执行`wget https://mirrors.bfsu.edu.cn/apache/tomcat/tomcat-8/v8.5.65/bin/apache-tomcat-8.5.65-fulldocs.tar.gz`下载  
- 执行`tar -zxvf apache-tomcat-8.5.65.tar.gz`解压  
...
- 4、打开Putty，进入新建用户的git项目文件夹下，执行**拉取的指令**，因nginx的配置文件有变化，所以要执行**重置和启动**指令方才生效，最后在浏览器输入**服务器ip或域名**/myvue，就能进入到部署的vue项目的主页
